...
def generate_master_dict(lists, impute_method='average'):
    """Generate master dictionary"""
    master_dict = {}
    rep_no = len(lists)

    for idx, file_name in enumerate(lists):
        cur_list = get_protein_list(file_name)
    ...
    master_dict = impute_missing_values(master_dict, rep_no, impute_method)
...
def get_protein_list(file_name):
    ...
    protein_info = [get_quant_data(x) for x in data]
